...
- [x] Тип кэша — **in-memory singleton**; persistent materialization в этой задаче не делается.
...
- `test_tnved_catalog_api.py -k children` на текущей ветке и `main` — одинаковые
  **5 failed** из-за отсутствующих кодов в пустой БД; новых падений нет.
- Полный `pytest tests/ -q --tb=no`: ветка — **1330 passed / 185 failed /
  13 errors / 2 skipped**, `main` — **1293 passed / 185 failed / 13 errors /
  2 skipped**. Набор failed/error идентичен; +37 passed на ветке — новые
  self-contained тесты этой задачи. Общие падения зависят от отсутствующих seed/data
  и неполного test-environment, а не от corrective diff.
- Полный auditor на локальной БД вернул `commodity_count=0`, `checked=0`,
