...
утверждённых ставок в новом контуре пока 0. Backend suite 1 726 passed до
финальных focused additions; frontend 37 passed, typecheck/build и HTTP smoke
пройдены. Подготовлен явный запуск на GitHub через `ett_acquire=true` либо
выделенную ветку `ops/ett-source-capture`. Исправлен прежний CI Run #15:
недопустимый runner-контекст в job env блокировал запуск заданий. Текущий raw-набор,
применимость сносок/дат и утверждение конкретного manifest ещё не завершены.
