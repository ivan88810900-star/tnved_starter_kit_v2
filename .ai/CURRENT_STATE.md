...
облака. Последний опубликованный этап: `d80d119`, дерево совпадает с локальным
`dbbd6e3`. PR #187 остаётся Draft. CI: 2 601 backend-тест, frontend tests /
...
PDF захватывается отдельно для проверки, прежняя диагностика не исправляется.
...
Batch остальных сканов выполняется, завершённость всего batch пока не заявлена.
