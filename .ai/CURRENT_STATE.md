...
## Актуальное продолжение, 14 сентября 2026

Реализован и независимо одобрен A5 ограниченный **AD30 executable review**:
24 неизменяемых факта сохранённых решений 12 → 4 → 121, продуктовые условия
и точный гипотетический расчёт явно выбранной строки через существующий ETT
preview. CLI работает изолированно; код ТН ВЭД проверяется вместе с товаром,
направлением, страной, формой и размерами. `as_of` явно сохраняется, но не
устанавливает юридический интервал. Итоговый платёж и legal applicability
остаются недоступными даже при успешной арифметике.

A5 выявил и независимо перепроверил две исправленные владельцами ошибки:
нормализованное название страны было ошибочно помечено буквальной транскрипцией;
самостоятельному денежному результату недоставало полной привязки строки и
единицы ставки. Исторические evidence не изменены. 43 независимых теста,
включая 15 реальных CLI-процессов с запретом попыток сетевых/DB/write-действий,
прошли; объединённый профиль — 449 passed. Точный опубликованный кандидат
`2bb519bf` прошёл CI 34824532766: backend 5 515 passed, 2 known skipped,
2 warnings, 85 subtests; frontend 50, TypeScript/build, staging и workflow
contracts успешны. Admission QA 34824532769: 2 322 passed, 1 known skipped,
2 warnings, 85 subtests. Старые результаты a9d15c74 не подменяют эти проверки.
[Publication evidence](../docs/ai-workflow/evidence/ad30-publication-20260914.json)
содержит точный tree и local→GitHub mapping; финальные feature HEAD и push/PR CI
после добавления этой документации фиксируются в PR #187.

Фактическое восстановление: PR #187 был на `a9d15c74`, base `a5a811e6`,
main `9712c7b`. Повторные восемь findings A6 уже исправлены/классифицированы
на этом HEAD; повторного изменения движка нет. Более поздние PR #191/#192
содержат CI matcher и source-fact документ: они переиспользованы, а не написаны
заново. Отдельные #190/#193, их runtime-state и #189 не изменяются.

A1/A2/A3 работали в отдельных новых worktrees от фактического HEAD; A5 был
независимым проверяющим, A4 документировал стабильный consumer contract.
Живые payment/NTM/AI API, source monitor и источники не переключены.
Количество источников прежнее: 49 registry / 49 policies / 72 default URLs
и 11 изолированных review targets. Это технический учёт, не legal coverage.

Следующий этап — полный source-bound review временной цепочки, поправок,
производителей и номенклатуры AD30, затем оставшиеся ЕТТ/НДС/акциз/защитные
меры/преференции/происхождение и продуктовые NTM исключения. Полный юридический
охват остаётся частичным; retention/legal hold, manifest-bound human review и
отдельное утверждение не подтверждены. DM-0014 не останавливает разработку.
Main/production DB/deploy/enforcement остаются без изменений.

[Текущий TASK](tasks/TASK-AD30-EXECUTABLE-REVIEW-001.md),
[CLI и границы результата](../docs/ai-workflow/AD30_EXECUTABLE_REVIEW.md),
[A5 evidence](../docs/ai-workflow/evidence/ad30-independent-qa-20260914.json).
Раздел ниже сохраняет итог 12 сентября; его «следующий этап» уже частично
реализован этим ограниченным review-блоком. Актуальный HEAD и checks — в PR #187.

---

## Исторический итог интеграции, 12 сентября 2026
