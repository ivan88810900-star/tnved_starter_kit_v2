...
облака. TASK-ETT-001 реализует локальные версионные кандидаты, проверку schema-v2,
пять таблиц и выбор ставки на явно заданную дату. Небезопасный старый PDF/OData
импорт и ложная свежесть по хешу индексной страницы отключены. 1 056 backend-тестов,
SQLite migration roundtrip и authenticated API smoke прошли. Это не готовая база
актуальных ставок. TASK-ETT-002 добавляет загрузку по реальному оглавлению,
неизменяемый технический receipt и проверку цитат по повторно извлечённым строкам
PDF. TASK-ETT-003 собирает полные ячейки, описания и переносы, отделяет
надстрочные сноски по геометрии PDF и сохраняет условия/даты примечаний. Исторический
сохранённый корпус: 96 PDF / 1 537 страниц / 13 289 реальных товарных строк;
все ячейки разобраны, включая автомобильные формулы. 29 упоминаний в тексте
примечаний исключены. Старый `0406900000` был дополнением заголовка `0406 90`
нулями в прежнем импорте; это не основание менять текущий каталог без проверки.
105 исторических примечаний не покрывают ссылки 106C–117C. Юридически
утверждённых ставок в новом контуре пока 0. Remote CI опубликованного `18aba33` прошёл: 1 750 backend-тестов,
frontend 37, typecheck/build и staging smoke. HTTP smoke также пройден локально. Подготовлен явный запуск на GitHub через `ett_acquire=true` либо
выделенную ветку `ops/ett-source-capture`. Исправлен прежний CI Run #15:
недопустимый runner-контекст в job env блокировал запуск заданий. Raw HTML текущего оглавления получен на GitHub; пустой дубль ссылки группы 24
обработан с сохранением проверки конфликтов. Run #34236950319 получил 100 PDF и индекс, затем остановился на старой ссылке
правового портала. Добавлены отдельный incomplete-capture отчёт с URL/SHA/временем
успешных запросов и анализ полного core-набора без выдачи его за complete receipt.
Инвентарь оглавления: 105 актов изменений, только 2 с прямыми ссылками; 103 без
ссылок и даты действия не достроены автоматически. Run #34239100895 сохранил
проверяемый incomplete-capture отчёт и все 100 core PDF; этот прежний результат
остаётся неполной загрузкой в истории.

Последующий [Run #34244241195](https://github.com/ivan88810900-star/tnved_starter_kit_v2/actions/runs/34244241195)
завершил поддерживаемый план технической загрузки, извлечение и анализ.
Проверен полный receipt v2
`ab67a2d3055a3fcd8416c874b9bc388ce0b0919e4b964e5de7c1f18c08a6f72e`:
100 core PDF, 2 страницы правового портала, 2 PDF-вложения и 2 снимка индекса —
106 оригиналов. Семантический план начального и конечного индекса совпадает;
побайтного совпадения HTML и атомарной юридической редакции это не доказывает.
Текущий анализ: 96 групп / 1 537 страниц / 13 293 реальных товарных строки,
124 примечания, 0 неразобранных ячеек/дубликатов/непривязанных номеров сносок.
Все 96 отчётов побайтно совпали с прежним локальным аудитом текущих PDF;
хеши парсеров сверены с коммитом запуска. Архив оригиналов сохранён отдельно
от временных GitHub artifacts; восстановление полного receipt проверено.
Доказательство: `docs/ai-workflow/evidence/ett-complete-capture-20260908.json`.

Это технически полная загрузка поддерживаемых ссылок, а не юридически полная
база ставок. Два PDF актов — сканы на 12 и 17 страниц без извлекаемого текста.
20 неподдержанных ссылок на вложения (11 уникальных URL, DOC/DOCX/ZIP) сохранены
в инвентаре, их содержимое этим запуском не загружено. Из 105 актов изменений
103 по-прежнему не имеют ссылки в исходном оглавлении. Юридическая интерпретация,
даты действия, проверка manifest и legal-retention attestation не завершены;
`legal_rates_resolved=0`, `legal_inventory_complete=false`, `production_ready=false`.
Облако, production promotion, merge/deploy и enforcement не включались.
Подробности: `docs/ai-workflow/ETT_VERSIONED_CANDIDATES.md`.
Новый контур: `docs/ai-workflow/ETT_TABLE_INTERPRETATION.md`.
