...
> Status: **Accepted with conditions** (Ivan, 2026-07-10)
...
#### 3.8.1 Тип кэша — решение для текущего этапа (APPROVE WITH NOTES)
- **Используем in-memory singleton cache** (один иммутабельный `CanonicalModel` в процессе).
- **Materialized persistent snapshot / файл / таблица в БД в TASK-CANONICAL-004 НЕ делаем.**
- **Причина:** минимальный blast radius и быстрый rollback — in-memory кэш не добавляет
  новых артефактов хранения (файлов/таблиц/миграций), которые пришлось бы версионировать,
  инвалидировать и откатывать; выключение serving-флага и/или сброс процесса полностью
  снимают эффект.
- **Persistent materialization остаётся будущим этапом** (отдельный ADR/TASK), если
  in-memory cache окажется дорогим по памяти/времени построения на prod-объёме (см. R6 и
  Open Decision «Materialized CanonicalModel» в `.ai/CURRENT_STATE.md §9`).
...
## 3a. Blocking gates (обязательны до первого `CANONICAL_TREE_ENABLED=1`)

> Уточнения по итогам Review (APPROVE WITH NOTES). Оба гейта — **жёсткие блокеры**: без их
> выполнения включать serving-флаг **запрещено в любом окружении** (в т.ч. dev/staging).

### Gate-1 — Revision-marker учитывает структуро-влияющие входы (`hs_rates`)
До первого включения `CANONICAL_TREE_ENABLED=1` **в любом окружении** revision / cache
invalidation **ОБЯЗАН** учитывать:
- **`tnved_commodities`** (набор и коды позиций);
- **`hs_rates`, влияющие на `leaf_flags` / `is_leaf_hs_code`** (leaf-флаги определяют
  synth-leaf/codeless-классификацию, т.е. **структуру** дерева).

Пока revision-marker не покрывает оба входа, serving-флаг включать **запрещено** (иначе —
stale-структура после ingestion ставок; R1/R2/R3). Только `db_codes` (текущий `snapshot_id`)
для этого гейта **недостаточно**.

### Gate-2 — Полный offline-обход даёт 0 mismatch
Перед включением serving-флага **полный offline-обход всех поддерживаемых `/children` кодов
на полной БД должен дать 0 mismatch** (structural + content) между legacy и canonical.
Живой/семплированный shadow — инструмент наблюдения, но **не** заменяет полный offline-обход
как критерий готовности.
...
- [x] Тип кэша: **in-memory singleton**; persistent materialization в TASK-004 не делается (§3.8.1).
- [x] Validator gate + fail-safe fallback на legacy без 500.
- [x] Rollback подтверждён: `CANONICAL_TREE_ENABLED=0` мгновенно возвращает legacy.
...
2. **Утвердить Gate-1 (revision-маркер кэша):** обязательный учёт `tnved_commodities` +
   `hs_rates` (leaf_flags) как **блокер** включения serving-флага (§3a; R1/R2).
3. **Утвердить Gate-2 (критерий serving ON):** полный offline-обход всех поддерживаемых
   `/children` кодов на полной БД = 0 mismatch (§3a).
4. **Тип кэша — решено на этот этап:** **in-memory singleton**; persistent materialized
   snapshot/file/table в TASK-004 **не делаем** (blast radius / rollback, §3.8.1).
   Persistent materialization — будущий этап при дороговизне in-memory (см. R6).
