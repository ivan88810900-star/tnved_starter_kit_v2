...
def _cursor_task_job_if_line() -> str:
...
def test_cursor_task_workflow_restricts_to_issues_only() -> None:
    job_if = _cursor_task_job_if_line()
    assert "github.event.label.name == 'cursor-task'" in job_if
    assert (
        "!github.event.issue.pull_request" in job_if
        or "github.event.issue.pull_request == null" in job_if
    ), f"job if must exclude PR-linked issues: {job_if!r}"
