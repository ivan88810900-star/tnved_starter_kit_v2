"""
Простой парсер для извлечения тарифных данных из PDF файлов
"""
import os
import re
import logging
...
def extract_tariff_data_simple(text: str) -> List[Dict[str, Any]]:
    """
    Простое извлечение тарифных данных из текста
    """
    tariff_records = []
    ...
    for line in lines:
        ...
        # Ищем строки с кодами ТН ВЭД - более гибкий подход
        # Ищем паттерн: 4 цифры + пробел + 2 цифры + пробел + 2 цифры + пробел + 2 цифры
        # Но также учитываем, что могут быть разные пробелы
        hs_code_pattern = r'(\d{4}\s+\d{2}\s+\d{2}\s+\d{2})'
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages):
                    ...
                    if text:
                        tariff_data = extract_tariff_data_simple(text)
