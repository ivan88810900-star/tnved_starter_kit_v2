...
- [ ] **Issue #137** — Улучшения калькулятора (страна происхождения с преференциями, утильсбор 8701–8705/8711, наглядное ИТОГО + экспорт PDF, история расчётов).
- [ ] **Issue #138** — Mobile responsive design (hamburger-меню, адаптивное дерево ТН ВЭД, вертикальная карточка, поля калькулятора в одну колонку, горизонтальный скролл таблицы платежей).
