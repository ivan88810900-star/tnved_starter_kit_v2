...
def _validate_official_anti_dumping_bundle_payload(
    ...
    for row_index, row in enumerate(measures):
        try:
            normalize_trade_remedy_rate(row, family_alias="antidumping")
        except ValueError as exc:
            return {
                "status": "parser_failed",
                "reason": "invalid_anti_dumping_rate_value",
...
def _normalize_measure_row(raw: dict[str, Any]) -> dict[str, Any] | None:
    ...
    rate_type, rate_percent, rate_specific = normalize_trade_remedy_rate(raw, family_alias="antidumping")
...
def run_anti_dumping_dry_run(*, rel_path: str | None = None) -> dict[str, Any]:
    ...
    response = AntiDumpingIngestionResponse(
        ...
        notes=[
            ...
            payment_admission_blocker("anti_dumping"),
...
def run_anti_dumping_apply(*, rel_path: str | None = None) -> dict[str, Any]:
    ...
    return _blocked_response(
        ...
        mode="apply",
        dry_run=False,
        blockers=[payment_admission_blocker("anti_dumping")],
        parser_result=parser_result,
        provenance=provenance,
        row_counts=AntiDumpingRowCounts(total_in_source=len(rows), blocked=len(rows)),
