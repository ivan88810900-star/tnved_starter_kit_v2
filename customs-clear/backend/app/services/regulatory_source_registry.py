...
TARIFF_RELIEF_SOURCE_REGISTRY: tuple[RegulatorySourceEntry, ...] = (
    RegulatorySourceEntry(
        ...
        title='Тарифные льготы — консолидированное Решение КТС №130',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/catr/ttr/preferences.php',
        description='Тарифные льготы по отдельным товарам и условиям; наблюдаемая консолидация.',
        monitor_urls=('https://eec.eaeunion.org/comission/department/catr/ttr/Duty%20Exemptions/%D0%9A%D0%A2%D0%A1%20130-2009%20(%D0%BE%D1%82%2009.07.2026%20%E2%84%96%E2%84%96%2075,%2077).pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Даты поправок №72/77 в консолидации расходятся с другими представлениями; контрольная сумма не разрешает конфликт.',
    ...
    RegulatorySourceEntry(
        source_id='eec_tariff_relief_728',
        title='Порядок тарифного освобождения — консолидированное Решение КТС №728',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/catr/ttr/preferences.php',
        description='Порядок применения освобождения от пошлины и подтверждающие документы.',
        monitor_urls=('https://eec.eaeunion.org/comission/department/catr/ttr/Duty%20Exemptions/%D0%9A%D0%A2%D0%A1%20728-2011%20(%D0%BE%D1%82%2009.07.2026%20%E2%84%96%2080).pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Оригинал №72 подтверждает 30.01.2026; заголовок №130 расходится с ним. Позднейшая поправка №80 к порядку требует отдельной проверки.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_gsp_conditions_47',
        title='Условия применения тарифных преференций — Решение Совета №47',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/dotp/tariff_preferences.php',
        description='Условия страны, товара и происхождения применяются совместно.',
        monitor_urls=('https://eec.eaeunion.org/upload/medialibrary/577/a261ucvbqfdjx4dah7wl5cun0jv3v8op/Polozhenie-ESTP.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Извлечение PDF не подтверждает право конкретной поставки на преференцию.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_gsp_beneficiary_countries',
        title='Перечни стран — пользователей тарифных преференций ЕАЭС',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/dotp/tariff_preferences.php',
        description='Раздельные перечни развивающихся и наименее развитых стран.',
        monitor_urls=('https://eec.eaeunion.org/upload/medialibrary/222/m3gyn7elrdc0hxo3j32w7k7vcwlf3wsi/Perechen-stran_polzovateley-15.05.26.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Членство страны не доказывает применимость преференции к товару или поставке.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_gsp_preferential_goods',
        title='Перечень преференциальных товаров — Решение Совета №8/2017',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/dotp/tariff_preferences.php',
        description='Перечни товаров, исключения и условия для двух режимов преференций.',
        monitor_urls=('https://eec.eaeunion.org/upload/medialibrary/1e5/z23azuqycqy567xgsu5awaukmdb2itvb/Perechen-preferentsialnkhykh-tovarov-_po-sostoyaniyu-na-08.03.2026_.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Код, наименование, исключения и редакция проверяются совместно; это не готовые правила расчёта.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_gsp_origin_60',
        title='Правила происхождения для преференций — Решение Совета №60',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/dotp/tariff_preferences.php',
        description='Происхождение, подтверждающие документы, транспортировка и административное сотрудничество.',
        monitor_urls=('https://eec.eaeunion.org/upload/medialibrary/dc8/kz4432n8enb7uyp83hshzcagsi3wm1gj/Pravila-proiskhozhdeniya-dlya-razvivayushchikhsya-i-naimenee-razvitykh-stran_GSP_obnovlen.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Сертификат не является единственным допустимым путём подтверждения; условия и исключения требуют проверки.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_gsp_treaty_article36',
        title='Тарифные преференции — официальный фрагмент статьи 36 Договора ЕАЭС',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/dotp/tariff_preferences.php',
        description='Общий размер преференций в зависимости от режима при выполнении его условий.',
        monitor_urls=('https://eec.eaeunion.org/upload/medialibrary/b02/Dogovor-EAES-_chast_.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Фрагмент договора не заменяет полный набор условий и актуальных исключений.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_gsp_authorized_issuers',
        title='Органы выдачи и заверения сертификатов происхождения формы А',
        authority_level="official_reference",
        official_url='https://eec.eaeunion.org/comission/department/dotp/tariff_preferences.php',
        description='Официальный справочный список органов стран-пользователей.',
        monitor_urls=('https://eec.eaeunion.org/upload/medialibrary/e1b/ncdlsvsx0yntf4h7zlsgjjx6p8vlu9r2/Spisok-organizatsiy-stran_benefitsiarov-_14.08.2026_.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
        known_gaps=(
            'Перечень органов не подтверждает подлинность конкретного сертификата и не заменяет верификацию.',
            "Отслеживается наблюдаемый PDF; замена ссылки на странице требует отдельного захвата и проверки.",
            "Мониторинг не обновляет тарифные ставки, страновые коэффициенты или обязательные меры.",
        ),
    ),
    RegulatorySourceEntry(
        source_id='eec_tariff_relief_council72_2026',
        title='Наблюдаемая публикация решения Совета №72/2026',
        authority_level="official_reference",
        official_url='https://docs.eaeunion.org/documents/461/10843/',
        description='Индивидуальный PDF, непосредственно связанный с карточкой 461/10843 правового портала.',
        monitor_urls=('https://docs.eaeunion.org/upload/iblock/0b3/9tdwg86ju0mmb7ye0zttsz77q2zq8o66/Reshenie-Soveta-_-72-ot-30-yanvarya-2026-g.pdf',),
        sync_script="monitor_official_ntm_sources.py",
        refresh_cadence="daily",
        max_age_hours=48,
        manual_review_default=True,
    ...
    *TARIFF_RELIEF_SOURCE_REGISTRY,
    ...
    RegulatorySourceEntry(
        ...
        monitor_urls=(
            ...
            "https://docs.eaeunion.org/upload/iblock/08a/wp70m6eckvicuanvf0sfo4sxqaro4aax/err_12022021_12_doc.pdf",
            "https://docs.eaeunion.org/upload/iblock/798/59qwa6jpe6b76n24wtn0uu9eygvlznkr/Reshenie-Kollegii-_-4-ot-20-yanvarya-2026-g.pdf",
            "https://docs.eaeunion.org/upload/iblock/75c/h3m62bw3jc8solzxiw14jc7qhmnycmmy/AD30R1_notice_fin.pdf",
            "https://remedies.eaeunion.org/dimd/filestorage/AD30R1_report_final.pdf",
        ),
        ...
        known_gaps=(
            ...
            "Мониторинг цепочки AD30 и финального отчёта фиксирует только доступность/изменение оригинала и не определяет юридический интервал или применимость.",
        ),
        manual_review_default=True,
    ),
    RegulatorySourceEntry(
