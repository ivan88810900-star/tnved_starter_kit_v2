"""Официальный excise ingestion: dry-run / guarded apply (issue #41)."""
...
_REGISTRY_SOURCE_CODE = "eec_excise"

_LOCAL_BUNDLE_CANDIDATES: tuple[str, ...] = (
    "data/raw_normative/eec_ett_excise.json",
)

# Только excise-поля hs_rates. duty_rate/source_* и vat_* не трогаем.
_EXCISE_APPLY_FIELDS = ("excise_type", "excise_value", "excise_basis", "valid_from", "valid_to")
...
def discover_excise_bundle_path(*, rel_path: str | None = None) -> str | None:
    """Найти локальный official excise bundle (не import-duty/VAT-only paths)."""
    if rel_path:
...
def _raw_row_has_excise_signal(raw: dict[str, Any]) -> bool:
    ...
    if ex_type in ("percent", "fixed"):
        return True
    if raw.get("excise_value") is not None:
        try:
            if float(str(raw.get("excise_value")).replace(",", ".")) > 0:
                return True
...
def _bundle_official_url(payload: dict[str, Any]) -> str:
    return str(
        payload.get("official_excise_url")
        or payload.get("official_ett_url")
...
def _build_provenance(
    *,
    ...
    return ExciseProvenance(
        ...
        legal_basis=entry.legal_basis if entry else "ЕТТ ЕАЭС / акцизные ставки при ввозе",
...
def run_excise_dry_run(*, rel_path: str | None = None) -> dict[str, Any]:
    ...
    if row_counts.blocked > 0 and row_counts.update == 0 and row_counts.skip == 0:
        dry_blockers.append(
            ...
            "(excise slice не создаёт duty rows)."
        )
...
def run_excise_apply(*, rel_path: str | None = None) -> dict[str, Any]:
    ...
    if row_counts.blocked > 0:
        ...
        apply_blockers.append(
            f"excise_rows_without_hs_rate: {row_counts.blocked} (excise slice не создаёт hs_rates)."
        )
        return _blocked_response(
            status="manual_review_required",
            mode="apply",
    ...
    response = ExciseIngestionResponse(
        ...
        notes=[
            "Excise slice: обновлены только excise_type/excise_value/excise_basis/valid_from/valid_to.",
            "Import-duty и VAT поля не изменяются.",
        ],
    )
    return response.model_dump(mode="json")
