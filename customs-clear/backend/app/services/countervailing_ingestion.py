...
def _validate_official_countervailing_bundle_payload(
    ...
    for row_index, row in enumerate(measures):
        try:
            normalize_trade_remedy_rate(row, family_alias="countervailing")
        except ValueError as exc:
            return {
                "status": "parser_failed",
                "reason": "invalid_countervailing_rate_value",
...
def _normalize_measure_row(raw: dict[str, Any]) -> dict[str, Any] | None:
    ...
    rate_type, rate_percent, rate_specific = normalize_trade_remedy_rate(raw, family_alias="countervailing")
