...
PAYMENT_SOURCE_REGISTRY: tuple[PaymentSourceEntry, ...] = (
    ...
    PaymentSourceEntry(
        ...
        name="НДС РФ — legacy bundle и разъяснения ФНС",
        domains=("vat",),
        authority_level="official_reference",
        official_url="https://www.nalog.gov.ru/rn77/taxation/taxes/nds/",
        legal_basis="Разъяснения ФНС со ссылками на НК РФ; применимость по коду требует исходных актов и review",
        local_canonical_paths=("data/raw_normative/eec_ett_vat.json",),
        source_status_code="EEC_VAT",
        loader_status="manual_review_required",
        sync_script="source_sync.py",
        target_tables=("hs_rates",),
        registry_source_id="rf_vat_tax_code",
        known_gaps=(
            "Обновляет только VAT-поля существующих hs_rates; не создаёт duty rows.",
            "EEC_VAT и source_code eec_ett_vat сохранены как legacy-идентификаторы, не как источник права ЕЭК.",
            "SourceStatus и checksum JSON не заменяют исходные артефакты и manifest-bound legal review.",
        ),
        manual_review_default=True,
    ),
    PaymentSourceEntry(
        source_code="eec_odata_vat_preferences",
        name="Портал открытых данных ЕАЭС — поиск источника льгот НДС",
        domains=("vat",),
        authority_level="official_reference",
        official_url="https://opendata.eaeunion.org/opendata/",
        legal_basis="Нормативное основание и endpoint набора льгот НДС пока не подтверждены",
        ...
        known_gaps=(
            "Подтверждён только официальный портал; конкретный VAT dataset/API и исходные акты не установлены.",
            "Legacy OData sync и tamdoc не заменяют юридическую проверку оригиналов и manifest.",
        ),
        manual_review_default=True,
    ),
    PaymentSourceEntry(
    ...
    PaymentSourceEntry(
        ...
        legal_basis="НК РФ / подзаконные акты по акцизам",
        local_canonical_paths=("data/raw_normative/eec_excise.json",),
        ...
        known_gaps=(
            ...
            "SourceStatus EEC_EXCISE и row-level stamps не заменяют исходные акты и manifest-bound legal review.",
            "Без локального eec_excise.json — missing_official_source.",
        ),
        manual_review_default=True,
    ),
    PaymentSourceEntry(
        ...
        local_canonical_paths=("data/raw_normative/eec_anti_dumping.json",),
        source_status_code="EEC_ANTI_DUMPING",
        loader_status="manual_review_required",
        target_tables=("special_duties",),
        registry_source_id="trade_remedies_official",
        known_gaps=(
            "Локальный canonical bundle кладётся в data/raw_normative/eec_anti_dumping.json; "
