...
const VOCABULARY: Record<string, Omit<PermitDescriptor, 'code'>> = {
  ...
  РЭВЧУ: {
    label: 'Разрешительный документ на ввоз РЭС и ВЧУ',
    organ: 'Роскомнадзор / Минпромторг России',
    severity: 'conditional',
  },
  'НФ/ЛЗ': {
    label: 'Нотификация ФСБ или лицензия на ввоз',
    organ: 'ЦЛСЗ ФСБ России / Минпромторг России',
    severity: 'conditional',
  },
  ВЕТКОНТРОЛЬ: {
    label: 'Ветеринарный контроль (вид документа уточняется)',
    organ: 'Россельхознадзор',
    severity: 'conditional',
  },
  'ФИТОКОНТРОЛЬ (БЕЗ ФСС)': {
    label: 'Фитосанитарный контроль без фитосанитарного сертификата',
    organ: 'Россельхознадзор',
    severity: 'conditional',
  },
  'ЛЗ/РАЗРЕШЕНИЕ ФСТЭК': {
    label: 'Лицензия или разрешение в сфере экспортного контроля',
    organ: 'ФСТЭК России',
    severity: 'conditional',
  },
  'ДС/СС': {
    label: 'Форма оценки соответствия уточняется',
    organ: 'Росаккредитация',
    severity: 'conditional',
  },
  'ДС/СС/СГР': {
    label: 'Форма подтверждения или регистрации уточняется',
    severity: 'conditional',
  },
  ЗАПРЕТ: {
    label: 'Запрет или ограничение перемещения (применимость уточняется)',
    severity: 'conditional',
  },
  CITES: {
    label: 'Разрешение или сертификат CITES',
    organ: 'Административный орган CITES',
    severity: 'conditional',
  },
};
...
function normalizeCode(raw: string | null | undefined): string {
  ...
  // CITES — международная латинская аббревиатура, а не опечатка кириллицы.
  if (c === 'CITES') return c;
