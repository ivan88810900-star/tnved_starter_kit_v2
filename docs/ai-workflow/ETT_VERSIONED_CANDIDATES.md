...
The schema requires exactly 96 chapter artifacts (01–76, 78–97), an index, nomenclature notes, tariff notes and an amendment inventory. The inventory may be a separate official artifact or an optional derived descriptor bound to the exact original index, supported parser identity and canonical report bytes. The latter mode uses 99 official artifacts and does not assign a fabricated URL to derived JSON. Stage and stored preview replay the entire inventory from original bytes. Existing v2 serialization and hashes remain unchanged when the descriptor is absent. A structurally present inventory is **not proof** that all current amendments are captured. Evidence binds an artifact hash, page/row locator and exact retained text hash; staging alone does **not** verify extraction of that quote from its PDF. TASK-ETT-002 adds a separate `verify-rows` check, which still does not approve legal interpretation. URL allowlisting is not download provenance.
...
python3 scripts/ett_candidates.py verify-evidence /path/to/candidate.json \
...
Continuation, 11 September 2026: [candidate duty arithmetic](ETT_DUTY_PREVIEW.md)
adds a read-only `preview-duty` CLI. It resolves an existing candidate, replays
its original quotations and calculates an exact, unrounded provisional amount
for the five supported formulas. It does not supply a legally approved rate,
authenticate currency factors or calculate a final payment.

1. [TASK-ETT-002](ETT_SOURCE_EVIDENCE.md) implements bounded official index acquisition, source-body retention, reproducible PDF rows and a candidate quote verifier. [Run #34244241195](https://github.com/ivan88810900-star/tnved_starter_kit_v2/actions/runs/34244241195) completed the supported technical capture with a verified v2 receipt: 100 core PDFs, two legal portal pages, two PDF attachments and both index captures, totaling 106 originals. The [complete-capture evidence](evidence/ett-complete-capture-20260908.json) verifies all 13,293 table rows from 96 chapter PDFs, 124 note identifiers and successful restoration of the source-only backup. No downloaded set directly updates `hs_rates`.
2. [TASK-ETT-003](ETT_TABLE_INTERPRETATION.md) assembles full code descriptions, complete rate cells and source-backed note clauses. Subsequent capture retained original PDFs for all 105 named amendments and founding Decision 80. OCR covers 430 pages (428 complete amendment pages and only two founding pages). A first real four-code 111C candidate now binds exact native quotations and typed legal-portal date metadata, independently reproduces its canonical SHA and passes isolated staging/authenticated API checks. The proposed dates, limited code lifetime and common-import scope remain explicit review interpretations. Complete applicability for 13,293 codes, all legal attachments and manifest-bound legal approval remain unfinished; `legal_inventory_complete=false` and `production_ready=false`. Reconstructing historical 2022–2026 law is a separate project.
3. [The deterministic review package](ETT_REVIEW_PACKAGE_READINESS.md) is implemented: exact candidate/prior bytes, full v2 core acquisition, selected additional-act capture bindings, recomputed source evidence and diff, and an isolated reproducible closure. Its CLI builds/verifies without database or network access. This is technical assembly, not manifest-bound legal approval. Legal review records, retention-capable storage attestation and atomic reviewed promotion, then compatibility materialization and product `as_of` integration remain outstanding. Architecture approval alone cannot satisfy these gates.
